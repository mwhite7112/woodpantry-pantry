...
type stageRequest struct {
	Items []service.StagedItemInput `json:"items"`
}

func handleStageJob(ingest *service.IngestService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		jobID, err := uuid.Parse(chi.URLParam(r, "job_id"))
		if err != nil {
			jsonError(r.Context(), w, "invalid job_id", http.StatusBadRequest)
		...
		result, err := ingest.StageItems(r.Context(), jobID, req.Items)
		if err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				jsonError(r.Context(), w, "job not found", http.StatusNotFound)
				return
